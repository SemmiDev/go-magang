...
func main() {
	...
	r.With(auth.MustLoginMiddleware(tokenMaker)).Get("/interns/delete", handlers.handleDeleteIntern)
...
func (h *handlers) handleDeleteIntern(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	id := query.Get("id")

	err := h.internRepo.DeleteIntern(id)
	if err != nil {
		url := fmt.Sprintf("/error?code=%d&message=%s", http.StatusInternalServerError, err.Error())
		http.Redirect(w, r, url, http.StatusSeeOther)
		return
...
func (h *handlers) handleCompletePersonalData(w http.ResponseWriter, r *http.Request) {
	...
	err = views[CompletePersonalDataView].Execute(w, intern)
	if err != nil {
		url := fmt.Sprintf("/error?code=%d&message=%s", http.StatusInternalServerError, err.Error())
		http.Redirect(w, r, url, http.StatusSeeOther)
		return
...
func (h *handlers) handleHomePage(w http.ResponseWriter, r *http.Request) {
	...
	if payload.Role == "admin" {
		...
		err = views[AdminDashboardView].Execute(w, internsData)
		if err != nil {
			url := fmt.Sprintf("/error?code=%d&message=%s", http.StatusInternalServerError, err.Error())
			http.Redirect(w, r, url, http.StatusSeeOther)
			return
	...
	err = views[InternDashboardView].Execute(w, intern)
	if err != nil {
		url := fmt.Sprintf("/error?code=%d&message=%s", http.StatusInternalServerError, err.Error())
		http.Redirect(w, r, url, http.StatusSeeOther)
		return
...
func (h *handlers) handleLoginPage(w http.ResponseWriter, r *http.Request) {
	err := views[LoginView].Execute(w, nil)
	if err != nil {
		url := fmt.Sprintf("/error?code=%d&message=%s", http.StatusInternalServerError, err.Error())
		http.Redirect(w, r, url, http.StatusSeeOther)
		return
...
func (h *handlers) handleGoogleCallback(w http.ResponseWriter, r *http.Request) {
	...
	// just enough check if intern has <full name>
	if intern.FullName != nil {
		http.Redirect(w, r, "/", http.StatusSeeOther)
		return
	}
...
func (h *handlers) handleErrorPage(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	...
	err := views[ErrorView].Execute(w, ErrMessage)
	if err != nil {
...
func (h *handlers) handleSuccessPage(w http.ResponseWriter, _ *http.Request) {
	err := views[SuccessView].Execute(w, nil)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
